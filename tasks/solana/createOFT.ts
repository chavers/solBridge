...
const DEFAULT_SHARED_DECIMALS = 2
...
console.log('DEFAULT_SHARED_DECIMALS', DEFAULT_SHARED_DECIMALS)
  ...
  .addOptionalParam('sharedDecimals', 'OFT shared decimals (default=6)', DEFAULT_SHARED_DECIMALS, devtoolsTypes.int)
    ...
    .setAction(
        async ({
          ...
          console.log('isMABA', isMABA)
            if (tokenProgramStr !== TOKEN_PROGRAM.toBase58() && !isMABA) {
          ...
          console.log('txBuilder', txBuilder.getInstructions()[2] );
            console.log('umi', umi );
          // txBuilder.getInstructions().forEach(instruction => {console.log(instruction)})
